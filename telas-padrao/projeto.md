...
- [ ] Fase 3 Conteudo      [ ] .publish-approved — 1/4 Money Pages publicada. `/telas_soldadas/` (molde_id 7) **APLICADA em produção em 2026-09-03**, com aprovação explícita da Priscila (repassada pela conta): yoast_title/yoast_desc reescritos, schema `Organization` (global) + `Product` injetados via arquivo em `wp-content/novamira-sandbox/schema-telas-soldadas.php` (hook `wp_head`), SEOPress desativado (conflito Yoast/SEOPress resolvido — Yoast é o único ativo agora). Tudo verificado ao vivo via `curl` (title/desc/schema renderizando corretamente na Home e na página). Faltam as outras 3 Money Pages da arquitetura aprovada para fechar a Fase 3: `/telas_onduladas/`, `/alambrado/` (a criar), `/tela-de-aco-inox/` (a criar) — ver `## Money Pages`.
- [ ] Fase 5 GMN
