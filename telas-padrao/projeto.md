...
| /telas_soldadas/ | — | tela soldada | 9.900/mes | 7 | **publicado em 2026-09-03** — yoast_title/yoast_desc reescritos, schema `Organization`+`Product` injetado (verificado ao vivo), SEOPress desativado (Yoast único ativo). Posição pré-otimização: 46/50 — acompanhar reindexação. |
| /telas_soldadas_modelos/ | — | modelos de tela | 720/mes | 489 | pendente (existe — posição 24) |
| /telas_onduladas/ | — | tela ondulada | 2.400/mes | 398 | **rascunho pronto, aguardando aprovação (2026-09-03)** — title/meta/schema Product escritos com conteúdo real (ver `fase3-conteudo/briefing-telas-onduladas.md`), NADA aplicado no WordPress ainda. Fora do top 20 nacional na SERP consultada (Fase 1) — checar indexação após publicar. |
...
- [x] Fase 2 Site — CONEXÃO RESOLVIDA em 2026-09-02. O CLI standalone `novamira` continua com o bug de Windows Credential Manager (não corrigido — issue aberto no GitHub `use-novamira/novamira-cli`), mas o site foi conectado por outra via: MCP nativo do Claude Code (`claude mcp add --transport http --scope user novamira-telaspadrao-com https://telaspadrao.com.br/wp-json/mcp/novamira-oauth` + autenticação OAuth própria do Claude Code, sem depender do Credential Manager). Com a conexão real ativa, preenchidos em 2026-09-02: `## Ambiente de Publicacao` (Novamira confirmado + abilities reais + tema/builder), `## Molde de Money Page` (inventário completo de `/telas_soldadas/`), `## Sitemap do cliente`, e as subseções "Schema do cliente" / "SEO Técnico do cliente" em `## Raio-X Tecnico`. Sem pendências do lado do cliente.
- [ ] Fase 3 Conteudo      [x] .publish-approved — Money Page `/telas_soldadas/` (molde_id 7): ETAPA 1 escolheu maior volume (9.900/mês)/pior posição (46º) como primeira página a trabalhar; `_elementor_data` completo lido, H2s dos 3 concorrentes extraídos — ver `fase3-conteudo/briefing-telas-soldadas.md`. Priscila aprovou em 2026-09-03: title/meta/schema Product como estavam no rascunho, aplicar schema `Organization`, manter Yoast SEO e desativar SEOPress. **APLICADO em produção em 2026-09-03:** yoast_title/yoast_desc reescritos, SEOPress desativado (Yoast confirmado como único plugin de SEO ativo), schema `Organization` (global) + `Product` injetados via `wp-content/novamira-sandbox/schema-telas-soldadas.php` (hook `wp_head`) — tudo verificado ao vivo via `curl` (Home e `/telas_soldadas/`). 1/4 Money Pages da arquitetura publicada — checkbox principal da Fase 3 só fecha quando `/telas_onduladas/`, `/alambrado/` e `/tela-de-aco-inox/` também forem tratadas.
- [ ] Fase 5 GMN
...
### Briefing de Otimização — /telas_soldadas/
> Detalhamento completo (conteúdo real da página, JSON-LD final) mora em `telas-padrao/fase3-conteudo/briefing-telas-soldadas.md` — não duplicado aqui. **APLICADO em produção em 2026-09-03** com aprovação explícita da Priscila (repassada pela conta): yoast_title/yoast_desc reescritos, schema `Organization`+`Product` injetados, SEOPress desativado. Ver `## Money Pages` e `## Estado das Fases` para o registro completo.
...
- **Faltando em ambas (diagnóstico original em 2026-09-02):** `Organization`/`LocalBusiness`, `Product`/`Service`, `FAQPage`, `AggregateRating`/`Review`. **Atualização 2026-09-03:** `Organization` (site-wide) e `Product` foram adicionados em `/telas_soldadas/` (aplicado em produção — ver `## Money Pages`/`## Estado das Fases`); `/telas_onduladas/` e as demais páginas continuam sem `Organization`/`Product` até serem trabalhadas na Fase 3. `FAQPage`/`AggregateRating` seguem ausentes em todo o site.
...
- Yoast SEO (28.4) e SEOPress (10.1) estavam ativos **simultaneamente** (diagnóstico em 2026-09-02). **Resolvido em 2026-09-03:** SEOPress foi desativado (decisão da Priscila, repassada pela conta), Yoast SEO é o único plugin de SEO ativo agora — verificado via `execute-php` (`is_plugin_active`).
...
- Tem site: ( x ) sim  ( ) nao
- Gap analysis: ATUALIZADO em 2026-09-02 com dados reais do Novamira (substitui a versão anterior baseada só em Ubersuggest). O site já tem páginas dedicadas para os 2 primeiros silos da arquitetura aprovada (`/telas_soldadas/`, `/telas_onduladas/`), construídas 100% em Elementor/Elementor Pro sobre o tema Hello Elementor (kit "Dustrium"), mas rankeiam muito mal (posição 46 para "telas soldada", fora do top 20 para "tela ondulada") apesar de conteúdo/estrutura já existirem. Causas técnicas confirmadas: falta schema `Organization`/`Product`/`Service`/`FAQPage` (só `WebPage`/`WebSite`/`BreadcrumbList` do Yoast), meta description não otimizada (auto-excerto), inconsistência de marca no `<title>` ("Telas Sob Medida" vs. NAP oficial "Telas Padrão"), Yoast+SEOPress ativos simultaneamente (risco de schema/meta duplicado), e conteúdo duplicado no sitemap (mesmo artigo em 4 slugs). Faltam completamente os silos "Tela de Aço Inox" e "Alambrado/Cercamento" (maior gap de volume).
- Veredito: ( x ) consertar  ( ) reconstruir — confirmado com inspeção real: plataforma (WordPress 7.1 + Elementor Pro) é sólida e a estrutura de silo já é compatível com a arquitetura aprovada. O problema é otimização on-page/SEO técnico, schema ausente e páginas faltantes — não a plataforma.
- Plano passo a passo: (1) resolver conflito Yoast/SEOPress (desativar um dos dois); (2) corrigir inconsistência de marca no `<title>`/`site_name` ("Telas Sob Medida" → "Telas Padrão"); (3) adicionar schema `Organization` (com NAP) e `Product`/`Service` nas páginas de produto; (4) reescrever meta description de `/telas_soldadas/` e `/telas_onduladas/` (hoje é auto-excerto); (5) resolver conteúdo duplicado do sitemap e decidir com a Priscila/cliente sobre os 3 posts órfãos do Elementor; (6) criar as 2 páginas de silo faltantes (Alambrado, Tela de Aço Inox) usando o molde de `/telas_soldadas/` (molde_id 7) como base — ver `## Molde de Money Page`.
