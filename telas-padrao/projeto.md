...
- [ ] Fase 3 Conteudo      [x] .publish-approved — Money Page `/telas_soldadas/` (molde_id 7): ETAPA 1 escolheu maior volume (9.900/mês)/pior posição (46º) como primeira página a trabalhar; `_elementor_data` completo lido, H2s dos 3 concorrentes extraídos, briefing com rascunho de `yoast_title`/`yoast_desc`/schema `Organization`+`Product` — ver `fase3-conteudo/briefing-telas-soldadas.md`. **Priscila aprovou em 2026-09-03**: title/meta/schema Product como está, aplicar schema `Organization`, manter Yoast SEO e desativar SEOPress. **PENDENTE — precisa de sessão com escrita no Novamira:** aplicar de fato no WordPress (desativar SEOPress, atualizar yoast_title/desc, injetar os 2 schemas), depois atualizar `## Money Pages` com status/data de publicação. Checkbox principal da Fase 3 só fecha quando isso for aplicado e as demais Money Pages da arquitetura (Telas Onduladas, Alambrado, Tela de Aço Inox) também forem tratadas.
- [ ] Fase 5 GMN
...
> Rascunho completo (conteúdo real da página, `yoast_title`/`yoast_desc`/schema `Product.description` finais) mora em `telas-padrao/fase3-conteudo/briefing-telas-soldadas.md` — não duplicado aqui para evitar duas cópias divergentes entre sessões. Resumo: title/meta/description já escritos com base no texto real da página (2026-09-03), nada aplicado no WordPress. Schema `Organization` (global) e a escolha Yoast vs. SEOPress seguem pendentes de decisão da Priscila.
